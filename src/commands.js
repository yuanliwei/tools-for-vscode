...
export const tool_commands = [
    ...
    {
        id: "y-transform-type-imports",
        label: "Transform type imports",
        async action(ed) {
            editText(ed, {}, async (text) => {
                return await transformTypeImportsText(text)
            })
        }
    },
    {
    ...
    {
        id: "y-copy-ide-context",
        label: "Copy IDE Context",
        async action(ed, args) {
            const result = await getActiveEditorContextText()
            await copyTextToClipboard(result)
        },
    },
    {
